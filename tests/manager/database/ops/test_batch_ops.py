...
def setup_data(session_local):
    ...
    file1 = File(display_name="file1.txt", storage_name="f1", tags=[], mime_type="text/plain", size=10)
    file2 = File(display_name="file2.txt", storage_name="f2", tags=[], mime_type="text/plain", size=20)
...
def test_add_and_get_batch(setup_data):
    ...
    batch = ops.add("b1", "created", ["f1", "f2"], "engine", "endpoint", "reader", "prompt", "model", 0.5)
...
def test_update_batch_file_status(setup_data):
    ops = BatchOps(setup_data)
    batch = ops.add("b1", "created", ["f1"], "e", "e", "r", "p", "m", 0.5)
    updated = ops.update_batch_file_status(batch["id"], "f1", "processed")
    with setup_data() as s:
        bf = s.query(BatchFile).filter_by(batch_id=batch["id"], storage_name="f1").first()
